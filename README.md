...
- Вода снимает `drunkenness.water-sobering` единиц опьянения **на единицу объёма** (по умолчанию 4). Алкогольные напитки добавляют опьянение; после порога оно вызывает только ванильную тошноту. Старинные предметы без тега `container` читаются; для старых зелий пустая тара по умолчанию — стеклянная бутылка.
...
## Опьянение: только ванильная тошнота

При питье алкогольного напитка растёт уровень опьянения (0–100). Он постепенно снижается, а вода уменьшает его на `drunkenness.water-sobering` за каждую единицу объёма тары. Единственный зельевой эффект, который BetterThanBrewery теперь выдаёт игроку, — **ванильная тошнота** (`NAUSEA`). По умолчанию она начинается с 20 единиц и усиливается после 80:
...
`/betterbrewery drunk <ник> 80` проверяет сильную тошноту, `/betterbrewery drunk <ник> 0` снимает тошноту, выданную плагином. При выходе эффект плагина снимается; после перезагрузки конфигурации новые пороги применяются к игрокам онлайн. Отображение ванильной тошноты зависит от настроек доступности клиента (искажение изображения).

**Ресурс-пак и библиотека эффектов удалены**: плагин больше не посылает пак, не рисует HUD/Title/actionbar, не меняет скорость или голос и не выдаёт бонусные зельевые эффекты из рецептов. Остаются питательность напитков и эффекты самой варки (звуки/частицы). Старые поля `settings.use-resource-pack-overlays`, `drunkenness.visual`, `drunkenness.stages`, `drunkenness.voice` и `output.effects` в уже скопированных файлах конфигурации/рецептов теперь игнорируются; при желании удалите их вручную. Старый ZIP ресурс-пака больше не нужен.
...
3. По необходимости установить CraftEngine, ItemsAdder, или Denizen.
