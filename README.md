...
## Тара: наполнение, перелив и употребление

Объём задан **целыми единицами**. У пустой тары есть ID, вместимость, собственный предмет заполненной тары и опциональная внешность из `output.item` рецепта:
...
containers:
  bottle:
    ...
    units: 1
    use-drink-item: true # если в рецепте output.item, показывать его
  cup:
    empty: minecraft:bowl
    filled: minecraft:potion
    units: 1
    use-drink-item: false # всегда использовать containers.cup.filled
  mug:
    empty: itemsadder:brewery:empty_mug
    filled: itemsadder:brewery:filled_mug
    units: 2
    use-drink-item: false
```

`mug` — **пример**, для него нужны реальные предметы ItemsAdder. Вместо `itemsadder:` можно использовать `craftengine:` или `minecraft:`. Предмет `filled` выбирается отдельно для каждой тары; `output.item` рецепта подменяет его *только* при `use-drink-item: true`. На бутылке это позволяет использовать уникальные текстуры рецептов, а кружка остаётся кружкой. Если в конфиге нет существующего ID пустого/полного предмета, разлив отклоняется **до вычитания жидкости**.

- На fluid-колонке пустая тара забирает ровно `units`. На водном входе бойлера/чайника пустая тара забирает столько же воды.
- Заполненная тара хранит `drink`, `age-weeks`, `alcohol`, `quality`, `units` **и `container`** в PDC. Вода тоже получает теги; её можно перелить обратно в водный резервуар. Декоративные иконки теги не получают.
- На входе дистиллятора/бочонка полный напиток возвращает **именно свою** пустую тару. Существующую жидкость можно доливать только *тем же* напитком: качество и возраст усредняются пропорционально объёму. Разные жидкости не смешиваются.
- Перелив «всё или ничего»: если кружка на 2 ед. не помещается в резервуар или ведру на 4 ед. осталось лишь 3 места, действие отменяется, исходный предмет **не расходуется**. Неизвестный рецепт не превращается автоматически в воду.
- Бутылка (`minecraft:potion`) пьётся с обычной анимацией: Paper заменяет выпитый предмет нужной пустой тарой в **использованной руке**. Не-питьевые кастомные предметы употребляются сразу правым кликом по воздуху. При стопке расходуется один предмет, пустая тара идёт в инвентарь; при полном инвентаре она выпадает рядом, а не исчезает. Работает и в левой руке.
- Вода снимает `drunkenness.water-sobering` единиц опьянения **на единицу объёма** (по умолчанию 4). Остальные напитки добавляют опьянение и эффекты рецепта. Старинные предметы без тега `container` читаются; для старых зелий пустая тара по умолчанию — стеклянная бутылка.

Источники воды настраиваются со своей вместимостью и возвратом тары. Ванильное зелье воды возвращает **стеклянную бутылку**, а не ведро:
...
water:
  source-items:
    ...
    - item: minecraft:potion_water
      units: 1
      empty: minecraft:glass_bottle
    # - item: craftengine:brewery:water_canister
    #   units: 3
    #   empty: craftengine:brewery:empty_canister
```

Старые списки из строк `minecraft:water_bucket` и `minecraft:potion_water` тоже читаются; для первого объём по-прежнему берётся из `water.bucket-units`. Для нестандартного источника обязательны `item`, `units` и `empty`.

## Опциональная HUD-виньетка для Java 26.2 (не размытие мира)

В комплекте `resource-pack/` лежит **настоящий ресурс-пак Java 26.2** (формат 88.0): четыре нарисованные полупрозрачные виньетки в шрифте `betterthanbrewery:intoxication`. Лёгкая стадия — тёплое золотистое свечение; средняя — янтарные и бирюзовые ореолы; сильная — двоящиеся розово-бирюзовые края; крайняя — густая сливовая дымка. **Центр прозрачен**, чтобы играть было удобно. В текстурах нет чужих ассетов; их можно воспроизвести скриптом `python3 tools/make_intoxication_pack.py` (Python без зависимостей).

### Как включить на сервере

1. Заберите артефакт `betterthanbrewery-intoxication-pack` из успешного GitHub Actions **или** соберите отдельно: `./gradlew resourcePack`. ZIP будет в `build/resource-pack/BetterThanBrewery-intoxication-26.2.zip`. Архив содержит `pack.mcmeta` **в корне**; его нельзя запаковывать в ещё одну папку. Не загружайте ZIP плагина вместо него.
2. Положите ZIP по **прямому публичному HTTPS URL**, доступному браузерам игроков без авторизации. Посчитайте SHA-1 **готового ZIP**: `sha1sum build/resource-pack/BetterThanBrewery-intoxication-26.2.zip` (в PowerShell: `Get-FileHash <файл.zip> -Algorithm SHA1`). При смене архива пересчитайте SHA-1.
3. Добавьте/измените следующие поля в `plugins/BetterThanBrewery/config.yml`:

   ```yaml
   settings:
     use-resource-pack-overlays: true
   drunkenness:
     visual:
       url: "https://example.org/BetterThanBrewery-intoxication-26.2.zip"
       sha1: "0123456789abcdef0123456789abcdef01234567" # замените настоящим SHA-1
       prompt: "BetterThanBrewery: атмосферный эффект опьянения"
       refresh-ticks: 40
   ```

4. Выполните `/betterbrewery reload` либо перезапустите сервер. Игроку будет предложено принять пак **один раз при входе**, а не при каждой смене стадии. Плагин добавляет свой пак поверх других (не заменяет серверный пак); рисует эффект **только после** статуса `SUCCESSFULLY_LOADED` именно своего UUID. Если пак отклонён, ссылка не работает или SHA-1 неверен, игрок всё равно видит actionbar и получает обычные эффекты, **битых символов на экране нет**.
5. Для проверки без варки администратор может выполнить `/betterbrewery drunk <ник> 80` (глубокая стадия), а затем `/betterbrewery drunk <ник> 0` (сразу убрать эффект). При обычной игре заполните тару на станции, выпейте несколько напитков; степень опьянения сохраняется и постепенно спадает. Вода помогает отрезветь.

Эта опциональная виньетка (по умолчанию выключена) — **полупрозрачный HUD через bitmap-глиф в Title**, а не размытие изображения мира. В Java 26.2 нет команды для индивидуального включения RP-постшейдеров; в [Java 26.3 появилась `/posteffect`](https://www.minecraft.net/en-us/article/minecraft-java-edition-26-3). Реализация настоящих шейдерных эффектов планируется **в отдельном плагине**, а не в BetterThanBrewery. Размер виньетки зависит от GUI-scale/формата экрана; на сверхшироком мониторе она может не дотянуться до краёв. Пока игрок пьян, плагин обновляет Title примерно раз в две секунды, поэтому другие плагины, использующие тот же Title, могут с ним конфликтовать. Можно отключить `settings.use-resource-pack-overlays` целиком или задать `overlay-glyph: ""` у отдельных стадий — actionbar, покачивание и эффекты продолжат работать. При протрезвлении, отказе от пака, выключении или смене конфигурации активная виньетка снимается; при выходе состояние пакета для этого игрока забывается. Пак не подменяет шлем и не требует клиентских модов.
...
- книгу рецептов;
- GUI-скелеты;
- параметры ресурс-пака и стадии опьянения.

```text
/betterbrewery drunk <игрок> <0..100>
```

Административная проверка/сброс опьянения. Поставьте `80`, чтобы посмотреть фильтр, затем `0`, чтобы убрать его. Права: `betterthanbrewery.admin`.
...
- all-or-nothing перелив, вместимость и средний возраст/качество жидкости;
- идентичность тары, пустой предмет после употребления и переполнение инвентаря;
- прозрачные текстуры и соответствие глифов конфигурации;
- запрос ресурс-пака только один раз и показ фильтра только после подтверждённой загрузки.

GitHub Actions также собирает и отдаёт клиентский ZIP отдельным артефактом. Запускает:
